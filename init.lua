...
now(function() -- Custom for Neovim
   ...
   vim.opt.shiftwidth = 3 -- Use this number of spaces for indentation
   ...
   nmap_leader('fm', '<Cmd>lua MiniFiles.open(vim.fn.stdpath("data").."/site/pack/deps/start/mini.nvim")<CR>',
      '[m]ini.nvim directory')
...
now(function() -- Plugins
   ...
   -- If not install by Mason Must define here !!!
   ...
   require("conform").setup({
      ...
      formatters_by_ft = {
         ...
         awk = { "awk" },
         lua = { "stylua" },
         perl = { "perltidy" }, -- "perlimports", Not work !!!
         python = { "isort", "black" },
         asm = { "asmfmt" },
         c = { "clang-format" },
         cpp = { "clang-format" },
   ...
   require("conform").formatters.perltidy = {
      args = {
         "-st",
         "--quiet",
         -- "--column_limit",
         -- "120",
         -- "--keep_column_alignment",
         -- "true",
         -- "--keep_simple_array",
         -- "true",
         -- "--keep_simple_hash",
   ...
   require("conform").formatters.perlimports = {
      args = { "-i" },
   }
   vim.api.nvim_create_autocmd("FileType", {
      pattern = advanced_filetype,
